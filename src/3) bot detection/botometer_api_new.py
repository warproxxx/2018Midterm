...
users = pd.read_csv('sure_shots.csv')

if (os.path.isfile('sure_shots_currentResults.csv')):
    resultDf = pd.read_csv('sure_shots_currentResults.csv')
    starting_account =  resultDf['username'].iloc[-1]
    
    starting_num = users[users['username'] == starting_account].index[0]
...
for screen_name, result in bom.check_accounts_in(users['username'].values):
    resultDf = resultDf.append({'username': screen_name, 'result': result}, ignore_index=True)
    resultDf.to_csv('sure_shots_currentResults.csv', index=False)
    print(i)
    i = i + 1
